"""
NEXUS VPS PANEL - VERCEL OPTIMIZED
Owner: PR4MOD_H4X
"""
import os
import json
...
# Vercel पर /tmp use करें (writable)
if os.environ.get("VERCEL"):
    DATA_DIR = Path("/tmp") / "nexus_data"
    FILES_ROOT = Path("/tmp") / "nexus_files"
else:
...
# ============================================
#  USER ROUTES (SIMPLIFIED FOR VERCEL)
# ============================================
@app.route("/dashboard")
@require_user
def user_dashboard():
...
# ============================================
#  VERCEL COMPATIBLE - DISABLED FEATURES
# ============================================
@app.route("/server/start", methods=["POST"])
@require_user
def server_start():
    return jsonify({"ok": False, "msg": "VERCEL: Process execution not available"})
...
def logs_api():
    return jsonify({
        "running": False,
        "file": None,
        "logs": ["⚠️ Vercel: Real-time logs not available"],
        "install": ["⚠️ Vercel: Package installation not available"]
    })

@app.route("/install", methods=["POST"])
@require_user
def install():
    return jsonify({"ok": False, "msg": "VERCEL: Package installation not available"})
...
<div class="feature-card"><i class="fas fa-bolt"></i><h3>Instant Deploy</h3><p>Under 1 second</p></div>
...
.vercel-notice{background:#fef3c7;border:2px solid #fde68a;border-radius:var(--radius-sm);padding:10px 16px;margin-bottom:12px;display:flex;align-items:center;gap:10px;font-size:12px;color:#92400e;font-weight:500}
.vercel-notice i{font-size:16px;color:#d97706}
...
<div class="wrap">
<div class="vercel-notice"><i class="fas fa-info-circle"></i> ⚡ Vercel Mode: Process execution disabled. Use Railway for full VPS features.</div>
<div class="grid">
<div class="card">
<div class="card-header"><i class="fas fa-server"></i> Server (Vercel Mode)</div>
<div class="status-row"><div class="status-dot stopped"></div><span class="status-label stopped">● Process Execution Unavailable</span></div>
<div style="font-size:11px;color:var(--text3);padding:8px;background:var(--bg);border-radius:6px;margin-bottom:10px;text-align:center">⚠️ For running scripts, deploy on Railway.app</div>
<div class="ctrl-group">
<select class="file-select" id="fileSelect" disabled style="opacity:0.6">
<option value="">— Vercel: Disabled —</option>
{% for f in files %}<option value="{{ f }}">{{ f }}</option>{% endfor %}
</select>
</div>
<div class="ctrl-group">
<button class="btn btn-outline btn-sm" disabled style="opacity:0.5"><i class="fas fa-play"></i> Start</button>
<button class="btn btn-outline btn-sm" disabled style="opacity:0.5"><i class="fas fa-stop"></i> Stop</button>
<button class="btn btn-outline btn-sm" disabled style="opacity:0.5"><i class="fas fa-sync"></i> Restart</button>
</div>
</div>
<div class="card">
...
<div class="grid">
<div class="card">
<div class="card-header"><i class="fas fa-cubes"></i> Install (Vercel: Disabled)</div>
<div class="install-row"><input type="text" id="installCmd" placeholder="pip install" disabled style="opacity:0.6"><button class="btn btn-primary" onclick="installModule()" disabled style="opacity:0.5"><i class="fas fa-download"></i></button></div>
<div class="terminal"><div class="term-bar"><span class="term-dot red"></span><span class="term-dot yellow"></span><span class="term-dot green"></span><span class="term-title">System</span></div><div class="term-body">⚠️ Vercel: Package installation not available</div></div>
</div>
<div class="card">
...
<div class="terminal" style="border:none;border-radius:0;margin-top:0"><div class="term-body" id="logOutput">⚠️ Vercel: Real-time logs not available</div></div>
</div>
</div>
</div>
<script>
...
function showToast(msg,type='success'){const el=document.createElement('div');el.style.cssText=`position:fixed;top:70px;right:16px;z-index:9999;padding:12px 20px;border-radius:var(--radius-sm);font-size:12px;font-weight:600;backdrop-filter:blur(20px);border:2px solid ${type==='success'?'#bbf7d0':'#fecaca'};background:${type==='success'?'#dcfce7':'#fee2e2'};color:${type==='success'?'#16a34a':'#dc2626'};animation:slideIn 0.3s ease;box-shadow:var(--shadow)`;el.textContent=msg;document.body.appendChild(el);setTimeout(()=>el.remove(),3000)}
...
function refreshLogs(){logOutput.innerHTML='⚠️ Vercel: Real-time logs not available'}
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("🚀 NEXUS VPS PANEL - VERCEL OPTIMIZED")
    print("="*70)
